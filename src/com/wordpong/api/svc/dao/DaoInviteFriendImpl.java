...
public class DaoInviteFriendImpl extends DaoBase<InviteFriend> implements
	...
	private static final Logger log = Logger
			.getLogger(DaoInviteFriendImpl.class.getName());
	...
	@Override
	public List<InviteFriend> getFriendInvitesByInviteeKey(User user)
			throws DaoException {
		List<InviteFriend> result = null;
		InviteFriendMeta e = InviteFriendMeta.get();
	...
	@Override
	public List<InviteFriend> getFriendInvitesByEmail(User user)
			throws DaoException {
		...
		if (user != null && user.getEmail() != null) {
			...
			try {
				result = Datastore.query(e)
						.filter(e.inviteeDetails.equal(user.getEmail()))
						.asList();
			} catch (Exception ex) {
				throw new DaoException("Err:" + ex.getMessage());
			}
	...
	public void withdrawInvitation(String inviteFriendKeyString)
			throws DaoException {
		Key k = KeyFactory.stringToKey(inviteFriendKeyString);
		delete(k);
	}
				...
				for (InviteFriend i : invites) {
					...
					if (i.getInviterKey().equals(key)) {
						try {
							...
							String m = "cancelInvitation failed. email:"
									+ email + " from:" + user + " Err:"
									+ ex.getMessage();
							log.warning(m);
	...
	public void ignoreInvitation(String friendInviteKeyStr) throws DaoException {
		...
		try {
			final String msg = "ignoreInvitation: keyStr:" + friendInviteKeyStr
					+ " key:" + k;
			...
			String m = "ignoreInvitation failed. keyStr:" + friendInviteKeyStr
					+ " Err:" + e.getMessage();
			log.warning(m);
			throw new DaoException(m);
		}
	...
	public InviteFriend getFriendInvite(String friendInviteKeyStr)
			throws DaoException {
	...
	@Override
	public void removeInvitation(Atomic at, InviteFriend fi)
			throws DaoException {
